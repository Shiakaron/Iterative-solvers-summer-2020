# -*- coding: utf-8 -*-
"""
Created on Wed Jul 29 16:01:09 2020
...
def main():
    ...
    for s in range(Nsteps):
        ...
        # compute CN_term from old solution
        CN_term = compute_rhs_pde()
        
        # newton krylov to update solution
...
def residual(u):
    """
    F(u) = dudt - (rhs(u_s+1) + rhs(u_s))/2
    dudt = (u_s+1 - u_s) / k
    rhs(u) = -(-Lap)^p(u) - lambda/(1+u)^2 + lambda*epsilon^(m-2)/(1+u)^m + lagrangian_term
    
    also,
    rhs(u_s) = prev_rhs, which we will save for the next iteration
    """
    # compute simple terms of new rhs
    new_rhs = - lambd_/((1+u)**2) + lambd_*(epsilon_**(m_-2))/((1+u)**m_)
    # compute laplacian or bilaplacian term
...
def compute_rhs_pde():
    """
    solve for dudt = -(-Lap)^p(u) - lambda/(1+u)^2 + lambda*epsilon^(m-2)/(1+u)^m + lagrangian_term
...
def lagrangian_term():
    """
    See appendix C
    lagrangian_term = Grad_x{u} dot Grad_ksi{Q_t}.
